...
subtitle: "State: Beta — Version: 0.1.3"
toc: true
header-includes:
  - \usepackage{titling}
...
- `<output_name>`: The output selector name as defined by the chip (e.g. BASE, PHASE)
...
<PIN_BLOCK> {
  <pin_name> = {
    ...
    fclk=<clock_name>,                      // required when mode=DIFFERENTIAL on OUT
    pclk=<clock_name>                       // required when mode=DIFFERENTIAL on OUT
  };
}
```
...
**Serialization Clock Attributes (`fclk`, `pclk`):**
...
*   The module port connected to a differential pin may have width equal to the chip's serializer ratio (e.g., 10 bits for a 10:1 serializer). The compiler handles the width conversion between the module port and the physical pin.
...
6. **Serialization Clocks:** `fclk` and `pclk` are required when `mode=DIFFERENTIAL` on output pins. Both must reference valid clock names declared in `CLOCKS` or generated by `CLOCK_GEN`.
