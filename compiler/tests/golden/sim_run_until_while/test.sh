...
if SIM_OUTPUT=$("$JZ_HDL" --simulate "${SCRIPT_DIR}/test.jz" --vcd -o /tmp/run_cond_golden.vcd 2>&1); then
...
# Test 2: Timeout produces runtime error and nonzero exit
if TIMEOUT_OUTPUT=$("$JZ_HDL" --simulate "${SCRIPT_DIR}/test_timeout.jz" --vcd -o /tmp/run_timeout_golden.vcd 2>&1); then
    echo "  FAIL: @run_until timeout should exit nonzero"
    echo "        output: $TIMEOUT_OUTPUT"
    FAIL=$((FAIL + 1))
else
    if echo "$TIMEOUT_OUTPUT" | grep -q "TIMEOUT.*run_until"; then
        echo "  PASS: @run_until timeout exits nonzero and reports TIMEOUT"
        PASS=$((PASS + 1))
    else
...
if VERBOSE_OUTPUT=$("$JZ_HDL" --simulate "${SCRIPT_DIR}/test.jz" --vcd -o /tmp/run_cond_verbose.vcd --verbose 2>&1); then
    ...
    echo "  FAIL: verbose simulation run exited nonzero"
    echo "        output: $VERBOSE_OUTPUT"
    FAIL=$((FAIL + 1))
fi
