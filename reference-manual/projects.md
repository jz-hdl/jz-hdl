---
url: /jz-hdl/reference-manual/projects.md
---
...
* clock & pin declarations (CLOCKS, IN\_PINS, OUT\_PINS, INOUT\_PINS),
...
* Import paths are resolved and normalized. Re-importing the same resolved file (directly or transitively) is a compile error (IMPORT\_FILE\_MULTIPLE\_TIMES).
...
* XLEN, ADDR\_WIDTH, feature enables, sizes shared across modules
* File paths for MEM initialization (firmware images, lookup tables)

Error examples

* Using CONFIG in runtime context → CONST\_USED\_WHERE\_FORBIDDEN / CONFIG\_USED\_WHERE\_FORBIDDEN
* Missing CONFIG name referenced in a module instantiation width → CONFIG\_USE\_UNDECLARED
* String CONFIG used as width → CONST\_STRING\_IN\_NUMERIC\_CONTEXT
...
* `clk_name` must be declared as an input pin (IN\_PINS or CLOCKS-matched)
...
* The input clock must refer to a clock declared in CLOCKS with a period (or a clock generated by another generator in the same CLOCK\_GEN block for chaining).
* The output name is a chip-specific output selector (e.g., `BASE`, `PHASE`, `DIV`, `DIV3`).
* The output clock must refer to a clock declared in CLOCKS **without** a period (the compiler computes it from the generator configuration).
* The generated clock must not be declared as an IN\_PIN and must not already be driven by another CLOCK\_GEN.
...
* A single CLOCK\_GEN block may contain multiple generators (e.g., a PLL followed by a CLKDIV that divides the PLL output).
...
* CLOCK\_GEN chaining is only permitted if the chip explicitly supports it.
...
A common pattern is to use a PLL to synthesize a high-frequency clock, then a CLKDIV to derive a lower-frequency clock from it. Both generators appear in the same CLOCK\_GEN block:
...
Three blocks declare the chip's electrical I/O: IN\_PINS, OUT\_PINS and INOUT\_PINS. Each entry names a logical pin and its electrical properties.
...
OUT_PINS {
  ...
  TMDS_CLK = { mode=DIFFERENTIAL, standard=LVDS25, drive=3.5 };
  TMDS_DATA[3] = { mode=DIFFERENTIAL, standard=LVDS25, drive=3.5 };
}
...
* `pull=UP` or `pull=DOWN` on an OUT\_PINS entry → `PIN_PULL_ON_OUTPUT`
* `term=ON` on a standard that doesn't support termination → `PIN_TERM_INVALID_FOR_STANDARD`

### Notes

* CLOCKS entries should reference an IN\_PINS clock pin name
...
* The bus\_id is a project-wide identifier and must be unique.
...
* **clock\_gen**: Array of objects describing clock generators (PLL, DLL, CLKDIV) with parameters, derived expressions, outputs, and constraints.

The JSON schema includes:

* `memory[].type`: `DISTRIBUTED` or `BLOCK`
* `memory[].configurations[]`: `{ width, depth }` pairs
* `clock_gen[].type`: `pll`, `dll`, or `clkdiv`
* `clock_gen[].parameters`: Named parameters with type, min, max constraints
* `clock_gen[].derived`: Computed values (e.g., VCO frequency) with expressions and valid ranges
* `clock_gen[].outputs`: Named outputs (e.g., `BASE`, `PHASE`, `DIV`, `DIV3`) with frequency expressions
...
The compiler validates MEM declarations and CLOCK\_GEN configurations against the chip data. If validation fails, a compile error is emitted with the specific constraint that was violated.
...
* For each port:
  ...
  * Otherwise the name must resolve to a declared pin in a compatible PIN block (IN → IN\_PINS/INOUT\_PINS/CLOCKS, OUT → OUT\_PINS/INOUT\_PINS, INOUT → INOUT\_PINS)
...
* Each const\_id must be unique inside its `@global` block
...
* Port/pin direction mismatch (e.g., module IN bound to OUT\_PINS, module INOUT bound to IN\_PINS) → error.
...
* \[ ] `CONFIG` contains shared sizes (XLEN, ADDR\_WIDTH) used by multiple modules
...
* Name clocks clearly (e.g., sys\_clk, ref\_clk) and document periods in CLOCKS.
...
Example with differential signaling and pull-up resistors (DVI output):
...
@project dvi_proj
  ...
  OUT_PINS {
    ...
    TMDS_CLK     = { mode=DIFFERENTIAL, standard=LVDS25, drive=4 };
    TMDS_DATA[3] = { mode=DIFFERENTIAL, standard=LVDS25, drive=4 };
  }
  ...
  @top dvi_top {
    IN  [1] clk       = SCLK;
    OUT [6] leds      = LED;
    OUT [1] tmds_clk  = TMDS_CLK;
    OUT [3] tmds_data = TMDS_DATA;
