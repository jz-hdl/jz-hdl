...
### PARSE.PARSE\_SYNTAX\_ERROR — Generic parse syntax error
...
### LEXICAL.ID\_SYNTAX\_INVALID — Identifier violates syntax
...
### LITERALS\_AND\_TYPES.LIT\_BARE\_INTEGER — Bare integer in runtime expression
...
### LITERALS\_AND\_TYPES.LIT\_UNDEFINED\_CONST\_WIDTH — Undefined CONST or CONFIG used as width
...
### TESTBENCH.TB\_BUS\_NOT\_FOUND — BUS definition not found

* Severity: ERROR
* Fix: A testbench `WIRE` block `BUS <bus_id> ...;` declaration must reference a `BUS` definition in scope.

### TESTBENCH.TB\_SETUP\_CLOCK\_ASSIGN — @setup assigns clock
