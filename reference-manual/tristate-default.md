---
url: /jz-hdl/reference-manual/tristate-default.md
---
...
* When `--tristate-default=GND` or `--tristate-default=VCC` is specified:
  ...
  * External INOUT\_PINS remain unchanged (their tri-state behavior is preserved for I/O pads).
...
| :--- | :--- | :--- |
| **TRISTATE\_TRANSFORM\_MUTUAL\_EXCLUSION\_FAIL** | Conditions are not provably mutually exclusive | Refactor to use explicit `IF`/`ELIF`/`ELSE` or add explicit guards to ensure exclusion |
| **TRISTATE\_TRANSFORM\_PER\_BIT\_FAIL** | Different bits have different driver sets | Restructure so all bits share the same drivers |
| **TRISTATE\_TRANSFORM\_BLACKBOX\_PORT** | Attempting to transform external blackbox port | Tri-state in opaque modules is preserved as-is |
| **TRISTATE\_TRANSFORM\_OE\_EXTRACT\_FAIL** | Cannot extract output-enable condition from a driver | Refactor driver to use a clear `condition ? data : z` pattern |
...
| :--- | :--- | :--- |
| **TRISTATE\_TRANSFORM\_SINGLE\_DRIVER** | Net marked as tri-state but only one driver present in execution | Remove unnecessary `z` assignment; net is not actually tri-state |
| **TRISTATE\_TRANSFORM\_UNUSED\_DEFAULT** | All execution paths covered; default value never used | Consider the transformation unnecessary; default is unreachable |
