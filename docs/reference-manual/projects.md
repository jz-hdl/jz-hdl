...
Semantics
- The input clock must refer to a clock declared in CLOCKS with a period (or a clock generated by another generator in the same CLOCK_GEN block for chaining).
...
- The generated clock must not be declared as an IN_PIN and must not already be driven by another CLOCK_GEN.
