...
<span class="line"><span>    IN   &lt;input_name&gt;  &lt;signal&gt;;</span></span>
<span class="line"><span>    OUT  &lt;output_name&gt; &lt;clock_signal&gt;;</span></span>
<span class="line"><span>    WIRE &lt;output_name&gt; &lt;signal&gt;;</span></span>
<span class="line"><span>    ...</span></span>
<span class="line"><span>    CONFIG {</span></span>
...
<span class="line"><span>  };</span></span>
<span class="line"><span>  ...</span></span>
<span class="line"><span>}</span></span></code></pre></div><p><code>&lt;gen_type&gt;</code> is one of: <code>PLL</code>, <code>DLL</code>, <code>CLKDIV</code>, <code>OSC</code>, <code>BUF</code>, or a numbered variant (e.g., <code>PLL2</code>). Numbered variants represent alternate implementations of the same function — for example, a chip may have both a <code>PLL</code> (integer dividers) and a <code>PLL2</code> (fractional dividers).</p><p>Semantics</p><ul><li><code>&lt;input_name&gt;</code>: The input selector name as defined by the chip data (e.g., <code>REF_CLK</code>, <code>CE</code>). The bound signal must refer to a clock declared in CLOCKS with a period (or a clock generated by another generator in the same CLOCK_GEN block for chaining).</li><li><code>&lt;output_name&gt;</code>: The output selector name as defined by the chip (e.g., <code>BASE</code>, <code>PHASE</code>, <code>DIV</code>, <code>DIV3</code>, <code>LOCK</code>). The keyword (<code>OUT</code> vs <code>WIRE</code>) must match the output&#39;s <code>is_clock</code> property in the chip data.</li><li><strong><code>OUT</code></strong> declares a <strong>clock output</strong>: must refer to a clock declared in CLOCKS <strong>without</strong> a period (the compiler computes it). Must not be declared as an IN_PIN and must not already be driven by another CLOCK_GEN. Only valid for outputs with <code>is_clock: true</code> in the chip data.</li><li><strong><code>WIRE</code></strong> declares a <strong>non-clock output</strong> (e.g., PLL lock indicator): must <strong>not</strong> be declared in the CLOCKS block. The compiler automatically declares the signal as a wire in the generated output. Only valid for outputs with <code>is_clock: false</code> in the chip data (e.g., <code>LOCK</code>).</li><li>CONFIG parameters are chip-specific and validated against the chip&#39;s generator definition.</li><li>Not all outputs or parameters need to be used; omitting them is acceptable.</li><li>A single CLOCK_GEN block may contain multiple generators (e.g., a PLL followed by a CLKDIV that divides the PLL output).</li></ul><h3 id="generator-types" tabindex="-1">Generator types <a class="header-anchor" href="#generator-types" aria-label="Permalink to “Generator types”">​</a></h3><ul><li><strong>PLL</strong> (Phase-Locked Loop): Frequency synthesis with VCO, feedback divider, and multiple outputs (BASE, PHASE, DIV, DIV3). Used for generating arbitrary frequencies from a reference clock. Numbered variants (e.g., <strong>PLL2</strong>) represent alternate PLL implementations on the same chip — for example, a variant with fractional dividers or additional outputs. CONFIG parameters and available outputs are chip-specific.</li><li><strong>DLL</strong> (Delay-Locked Loop): Phase alignment without frequency multiplication. Used for clock de-skew.</li><li><strong>CLKDIV</strong> (Clock Divider): Simple fixed-ratio frequency division. Divides an input clock by a fixed ratio (e.g., 2, 3.5, 4, 5). No VCO or feedback — simpler and lower-power than PLL. Produces a single BASE output. The <code>MODE</code> CONFIG parameter selects the chip-specific variant (e.g., <code>local</code> for IO-logic dividers, <code>global</code> for fabric-wide dividers) when multiple variants are available.</li><li><strong>OSC</strong> (Internal Oscillator): On-chip RC oscillator that generates a clock without any external reference. Does not require an IN clock. The output frequency is determined by chip-specific CONFIG parameters (e.g., <code>FREQ_DIV</code>, <code>DEVICE</code>). Produces a single BASE output.</li><li><strong>BUF</strong> (Clock Buffer): Buffers an input clock onto a global clock network without frequency change. Used to route external clocks through dedicated clock routing resources.</li></ul><p>Validation</p><ul><li>Input clock must have a declared period in CLOCKS (or be driven by a prior generator in the same block).</li><li>Output clocks must not have a period in CLOCKS (it is derived).</li><li>VCO frequency (for PLL; derived from reference clock and divider parameters) must be within the chip&#39;s valid range.</li><li>CONFIG parameter values must match their declared type: integer parameters reject decimal values (e.g., <code>CLKFBOUT_MULT = 5.0</code> is an error; use <code>5</code>), while double parameters accept both (e.g., <code>CLKFBOUT_MULT_F = 5.125</code>).</li><li>CLOCK_GEN chaining is only permitted if the chip explicitly supports it.</li></ul><h3 id="pll-example" tabindex="-1">PLL example <a class="header-anchor" href="#pll-example" aria-label="Permalink to “PLL example”">​</a></h3><div class="language-text"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark" style="--shiki-light:#24292e;--shiki-dark:#e1e4e8;--shiki-light-bg:#fff;--shiki-dark-bg:#24292e;" tabindex="0" dir="ltr"><code><span class="line"><span>CLOCKS {</span></span>
...
<span class="line"><span>    IN REF_CLK sys_clk;</span></span>
<span class="line"><span>    OUT BASE fast_clk;</span></span>
<span class="line"><span>    WIRE LOCK pll_lock;          // non-clock output: PLL lock indicator</span></span>
<span class="line"><span></span></span>
<span class="line"><span>    CONFIG {</span></span>
...
<span class="line"><span>    }</span></span>
<span class="line"><span>  };</span></span>
<span class="line"><span>}</span></span></code></pre></div><p>The <code>WIRE LOCK pll_lock</code> line declares the PLL&#39;s lock indicator as a wire signal. This signal is typically referenced by differential output pins via the <code>reset</code> attribute (see <a href="#pin-blocks">PIN blocks</a>).</p><h3 id="clkdiv-example" tabindex="-1">CLKDIV example <a class="header-anchor" href="#clkdiv-example" aria-label="Permalink to “CLKDIV example”">​</a></h3><div class="language-text"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark" style="--shiki-light:#24292e;--shiki-dark:#e1e4e8;--shiki-light-bg:#fff;--shiki-dark-bg:#24292e;" tabindex="0" dir="ltr"><code><span class="line"><span>CLOCKS {</span></span>
...
<span class="line"><span>    IN REF_CLK serial_clk;</span></span>
...
<span class="line"><span>CLOCK_GEN {</span></span>
<span class="line"><span>  PLL {</span></span>
<span class="line"><span>    IN REF_CLK SCLK;</span></span>
<span class="line"><span>    OUT BASE serial_clk;</span></span>
<span class="line"><span>    WIRE LOCK pll_lock;</span></span>
<span class="line"><span>    CONFIG {</span></span>
...
<span class="line"><span>  TMDS_CLK = { mode=DIFFERENTIAL, standard=LVDS25, drive=3.5, fclk=serial_clk, pclk=pixel_clk, reset=pll_lock };</span></span>
<span class="line"><span>  TMDS_DATA[3] = { mode=DIFFERENTIAL, standard=LVDS25, drive=3.5, fclk=serial_clk, pclk=pixel_clk, reset=pll_lock };</span></span>
<span class="line"><span>}</span></span>
<span class="line"><span></span></span>
<span class="line"><span>INOUT_PINS {</span></span>
...
<span class="line"><span>    PLL {</span></span>
<span class="line"><span>      IN REF_CLK SCLK;</span></span>
<span class="line"><span>      OUT BASE serial_clk;       // 185.625 MHz (5x pixel clock)</span></span>
<span class="line"><span>      WIRE LOCK pll_lock;        // lock indicator → used as serializer reset</span></span>
<span class="line"><span>      CONFIG {</span></span>
<span class="line"><span>        IDIV = 7;</span></span>
<span class="line"><span>        FBDIV = 54;</span></span>
<span class="line"><span>        ODIV = 4;</span></span>
<span class="line"><span>      };</span></span>
<span class="line"><span>    };</span></span>
<span class="line"><span>    CLKDIV {</span></span>
<span class="line"><span>      IN REF_CLK serial_clk;</span></span>
<span class="line"><span>      OUT BASE pixel_clk;        // 185.625 / 5 = 37.125 MHz</span></span>
<span class="line"><span>      CONFIG {</span></span>
<span class="line"><span>        DIV_MODE = 5;</span></span>
<span class="line"><span>      };</span></span>
<span class="line"><span>    };</span></span>
...
<span class="line"><span>    IN  [1]  clk       = pixel_clk;</span></span>
<span class="line"><span>    OUT [6]  leds      = LED;</span></span>
<span class="line"><span>    OUT [10] tmds_clk  = TMDS_CLK;</span></span>
<span class="line"><span>    OUT [10] tmds_d0   = TMDS_DATA[0];</span></span>
<span class="line"><span>    OUT [10] tmds_d1   = TMDS_DATA[1];</span></span>
<span class="line"><span>    OUT [10] tmds_d2   = TMDS_DATA[2];</span></span>
<span class="line"><span>    INOUT [1] edid_dat = EDID_DAT;</span></span>
<span class="line"><span>    INOUT [1] edid_clk = EDID_CLK;</span></span>
<span class="line"><span>  }</span></span>
<span class="line"><span></span></span>
<span class="line"><span>@endproj</span></span></code></pre></div><p>In this example, <code>WIRE LOCK pll_lock</code> declares the PLL&#39;s lock indicator as a wire. The <code>reset=pll_lock</code> attribute on the TMDS pins tells the compiler to hold the serializer in reset until the PLL is locked. The compiler automatically inverts <code>pll_lock</code> for the active-low serializer reset.</p><p>Example showing CONFIG + top instantiation using CONFIG:</p><div class="language-text"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark" style="--shiki-light:#24292e;--shiki-dark:#e1e4e8;--shiki-light-bg:#fff;--shiki-dark-bg:#24292e;" tabindex="0" dir="ltr"><code><span class="line"><span>@project param_proj</span></span>
...
<span class="line"><span>  }</span></span>
<span class="line"><span>@endproj</span></span></code></pre></div>`,202)])])}const m=s(l,[["render",p]]);export{u as __pageData,m as default};
