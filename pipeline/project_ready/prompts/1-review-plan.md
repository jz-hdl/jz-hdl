...
The user wants a full 1.0.0 release-readiness review of the `jz-hdl-dev` project. This is a meta-task: the work itself is reading the codebase and producing `<OUTPUT_FILE>` at the project root. No source code is modified.

Deliverable: `<OUTPUT_FILE>` — a markdown file that scores each logical unit of the project 1–10 against 1.0.0 release quality, explains the score, lists high-level items outstanding before 1.0.0, and gives an overall project score with rationale.
...
3. Overall review → aggregate judgment, explain how the overall rank was reached. On a re-run, this section is recomputed and replaced.
...
Before doing any work, check whether `<OUTPUT_FILE>` already exists.
...
1. **Read the current `<OUTPUT_FILE>`** to capture its full text (it has been updated with every prior group's completed section).
2. **Dispatch a general-purpose `Agent`** with a fully self-contained prompt containing:
   ...
   - **The current full text of `<OUTPUT_FILE>`** — so the agent can see which groups have already been reviewed, what scores they received, and stay calibrated with the established tone and stringency. The agent is told: "use the prior groups as calibration anchors — do not invent your own scale."
   ...
   - A hard rule: **return only the completed markdown section**, no commentary, no preamble, no summary — it will be pasted directly into `<OUTPUT_FILE>`.
...
4. **Edit `<OUTPUT_FILE>`** to replace the group's section with the validated text (using `Edit` with the previous section as `old_string` and the new section as `new_string` — this preserves surrounding structure). Update the status header to show which groups are complete.
...
6. **Edit** `<OUTPUT_FILE>` to add/replace the overall section, the calibration adjustments, and the consolidated blocker list. Update status header to "Review complete" with today's date and note "(re-run; previous review: <prior date>)" if applicable.
...
- `<OUTPUT_FILE>` — created in Step 1, edited in every sub-part of Step 2, edited again in Step 3.
