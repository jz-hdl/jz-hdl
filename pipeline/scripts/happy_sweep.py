#!/usr/bin/env python3
"""Happy-path sweep runner: drive 5-happy-sweep.md one test plan at a time.

The sweep prompt at `pipeline/prompts/tests/5-happy-sweep.md` is written to
process a single plan's `### Missing Happy-Path` entries in
`compiler/tests/issues.md`. This runner drives it per plan so a rate-limit
interruption can be resumed with `--start N` the same way `context_sweep.py`
...
Usage:
    pipeline/scripts/happy_sweep.py                       # sweep every plan
    pipeline/scripts/happy_sweep.py --filter test_4_     # subset
    pipeline/scripts/happy_sweep.py --start 32           # resume at plan 32
    pipeline/scripts/happy_sweep.py --start-at test_6_4  # resume by substring
    pipeline/scripts/happy_sweep.py --list               # show targets, exit
    pipeline/scripts/happy_sweep.py --dry-run            # show invocations
"""
...
PROMPT_FILE = os.path.join(PIPELINE_DIR, "prompts", "tests", "5-happy-sweep.md")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run pipeline/prompts/tests/5-happy-sweep.md against each test "
