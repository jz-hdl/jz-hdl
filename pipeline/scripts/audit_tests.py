#!/usr/bin/env python3
"""Audit existing validation tests by running 3-audit.md against each test plan.

Drives Claude Code (via `claude -p`) over one or more `pipeline/test_*.md`
files. The audit prompt is read from `pipeline/prompts/tests/3-audit.md` and
the `<TEST_PLAN_FILENAME>` placeholder is replaced with the current plan's
basename (without the `.md` suffix) before invocation.

Findings are appended to `compiler/tests/issues.md` by the audit prompt itself;
this script only orchestrates the runs and reports pass/fail.

Usage:
    pipeline/scripts/audit_tests.py                       # audit every plan
    pipeline/scripts/audit_tests.py --filter test_4_     # subset
    pipeline/scripts/audit_tests.py --start-at test_6_4  # resume
    pipeline/scripts/audit_tests.py --list               # show targets, exit
    pipeline/scripts/audit_tests.py --dry-run            # show invocations
"""
...
PROMPT_FILE = os.path.join(PIPELINE_DIR, "prompts", "tests", "3-audit.md")
PLACEHOLDER = "<TEST_PLAN_FILENAME>"


def load_prompt(test_filename: str) -> str:
    """Load the audit prompt and substitute the test plan filename.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Audit existing validation tests by running pipeline/prompts/tests/"
            "3-audit.md against each test plan."
        ),
    )
    parser.add_argument(
        "--filter",
        type=str,
        default=None,
        help="Only audit test files whose basename contains this substring "
        "(e.g. 'test_4_1').",
    )
    parser.add_argument(
        "--start",
    ...
    if start_offset > 0:
        print(
            f"Found {len(test_files)} test plan(s) to audit "
            f"(plans {start_offset + 1}-{total_matched} of {total_matched}).\n"
        )
    else:
        print(f"Found {len(test_files)} test plan(s) to audit.\n")
    ...
    print("=" * 60)
    print(f"Done. {len(results['pass'])} audited, {len(results['fail'])} failed.")
    if results["fail"]:
        print("\nFailed:")
